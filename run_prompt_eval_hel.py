...
# nohup python prompt/run_prompt_eval_hel.py > /home/chenxing/opencompass/log/llama3/prompt/test/hellaswag_sft_random_prompt_1_3epoch_1_24_prompt.log &
# nohup python prompt/run_prompt_eval_race.py > /home/chenxing/opencompass/log/llama3/prompt/test/hellaswag_sft_random_prompt_1_3epoch_1_24_prompt.log &
def run_command_with_gpu(prompt, gpu_id):
    ...
    command = [
        ...
        'python3 run.py '
        f'--datasets hellaswag_gen_6faab5-{gpu_id} '
        '--hf-type base '
        '--hf-path /home/chenxing/Meta-Llama-3-8B '
        '--peft-path /home/chenxing/model/hellaswag_sft_TopAccuracy_prompt ',
        '--num-gpus 1 '
        '--debug'
    ]
    try:
...
def split_prompts(file_path, num_parts=8):
    ...
    if 'text' not in df.columns:
        raise ValueError("Excel 文件中没有 'text' 列。")
...
def run_prompt_part(gpu_id, prompt_part, ws):
    ...
    file_path = f"/home/chenxing/opencompass/configs/datasets/hellaswag/hellaswag_gen_6faab5-{gpu_id}.py"
        
    line_number = 19
    results = []
    for prompt in prompt_part:
        new_line_content = f"""                prompt= "{str(prompt)}","""
...
if __name__ == "__main__":
    prompt_path = "/home/chenxing/opencompass/prompt/robust_testdataset_hel.xlsx"
    save_path = "/home/chenxing/opencompass/prompt/robust_testdataset_hellaswag_sft_TopAccuracy_prompt_result_time.xlsx"
    # 给每个GPU分一块
    wb = Workbook()
    ws = wb.active
    ...
    with ThreadPoolExecutor(max_workers=num_parts) as executor:
        futures = []
        for gpu_id in range(num_parts):
            # 为每个 GPU 分配一个任务
            future = executor.submit(run_prompt_part, gpu_id, prompt_parts[gpu_id], ws)
