...
# nohup python prompt/run_prompt_eval_race.py > /home/chenxing/opencompass/log/llama3/prompt/test/race_random_prompt_1_3epoch_result_time.log &
...
def run_command_with_gpu(prompt, gpu_id):
    ...
    command = [
        ...
        'python3 run.py '
        f'--datasets race_gen_69ee4f-{gpu_id} '
        '--hf-type base '
        '--hf-path /home/chenxing/Meta-Llama-3-8B '
        #'--peft-path /home/chenxing/model/race_qvproj_lora_sft ',
        '--peft-path /home/chenxing/model/race_sft_TopAccuracy_prompt ',
        '--num-gpus 1 '
        '--debug'
    ]
    try:
...
def run_prompt_part(gpu_id, prompt_part, ws):
    ...
    file_path = f"/home/chenxing/opencompass/configs/datasets/race/race_gen_69ee4f-{gpu_id}.py"
        
    line_number = 22
    results = []
    for prompt in prompt_part:
        #                'Read the article, and answer the question by replying A, B, C or D.\n\nArticle:\n{article}\n\nQ: {question}\n\nA. {A}\nB. {B}\nC. {C}\nD. {D}'
        new_line_content = f"""                "{prompt}" """
...
if __name__ == "__main__":
    prompt_path = "/home/chenxing/opencompass/prompt/robust_testdataset_race.xlsx"
    save_path = "/home/chenxing/opencompass/prompt/robust_testdataset_race_sft_ZOPO_prompt_result_time.xlsx"
    # 给每个GPU分一块
    wb = Workbook()
    ws = wb.active
