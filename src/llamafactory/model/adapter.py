...
from peft import LoraConfig, LoraModel, PeftModel, TaskType, get_peft_model, AdaLoraConfig, VeraConfig
...
def init_adapter(
    ...
    if finetuning_args.finetuning_type == "adalora":
        ...
        model = get_peft_model(model, lora_config)
        return  model  
    if finetuning_args.finetuning_type == "olora":
        logger.info("Fine-tuning method: OLoRA")
        lora_config = LoraConfig(
            init_lora_weights="olora"
        )
        model = get_peft_model(model, lora_config)
        return  model 
    if finetuning_args.finetuning_type == "vera":
        logger.info("Fine-tuning method: VeRA")
        lora_config = VeraConfig(
            r=finetuning_args.lora_rank,
            vera_dropout=finetuning_args.vera_dropout,
            target_modules=finetuning_args.target_r,
        )
        model = get_peft_model(model, lora_config)
        return  model 
    if finetuning_args.finetuning_type == "rosa":
        logger.info("Fine-tuning method: Rosa")
        from peft_rosa.tuners.rosa import RosaConfig
        lora_config = RosaConfig(
            task_type=TaskType.SEQ_2_SEQ_LM,
            r=finetuning_args.lora_rank,
            lora_alpha=finetuning_args.lora_alpha,
            lora_dropout=finetuning_args.lora_dropout,
            target_modules=finetuning_args.target_r,
            d=finetuning_args.density,                            
            spa_num_grads=1,                        
    ...
    if finetuning_args.finetuning_type == "loraprune":
        logger.info("Fine-tuning method: LoRAPrune")
        lora_config = LoraConfig(
            task_type=TaskType.SEQ_2_SEQ_LM,
            r=finetuning_args.lora_rank,
            lora_alpha=finetuning_args.lora_alpha,
